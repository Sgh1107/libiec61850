...
{
    const char* peerAddress = ClientConnection_getPeerAddress(connection);

    if (connected) {
        activeConnections++;
        printf("Connection opened: client=%p peer=%s\n",
            (void*) connection,
        ...
        printf("Connection closed: client=%p peer=%s\n",
            (void*) connection,
            peerAddress ? peerAddress : "unknown");
    }
...
{
    // usage: ./sever_example_basic_io port
    int tcpPort = 102;
    if (argc > 1) {
        tcpPort = atoi(argv[1]);
    }
    ...
    // 动态数据集允许客户端在运行时创建、修改或删除数据集，而不是只能使用 CID 中预先定义好的静态数据集
